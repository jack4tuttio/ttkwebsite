  ...
  return text
    ...
    .map((line, index) => {
      ...
      if (parts.length < 3 || parts.length > 5) {
        throw new Error(
          `Invalid flagged.txt entry on data line ${index + 1}. ` +
          'Expected: USER_ID|CATEGORIES|SUMMARY|DATE|STATUS'
        );
      }
      ...
      if (!/^\d+$/.test(userId)) {
        throw new Error(
          `Invalid Roblox user ID "${userId}" on data line ${index + 1}`
        );
      }

      if (!categoryText || !summary) {
        throw new Error(
          `Missing category or summary on data line ${index + 1}`
        );
      }
...
async function fetchRobloxProfile(userId) {
  ...
  try {
    ...
    return {
      ...
      displayName: user.displayName || user.name || fallback.displayName,
      created: user.created || null,
      description: user.description || '',
      profileAvailable: true
    };
  } catch (error) {
    console.warn(`Could not load Roblox user ${userId}: ${error.message}`);
    return fallback;
  }
}
...
async function build() {
  console.log(`Reading ${FLAGGED_FILE}...`);
